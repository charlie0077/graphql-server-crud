...
class ModelBase {
    ...
    searchQuery (args) {
      ...
      // default sort by id
      args.orderBy = args.orderBy || [{ column: this.uniqueColumn }]
    ...
    async executeQuery (sql, type, args) {
      ...
      this.checkQueryArgs(sql, type, args)
    ...
    checkQueryArgs (sql, type, args) {
      if (['SEARCH'].includes(type)) {
        // if aggregation query has nested fields, then uniqueColumn should be in the group by fields
        const nestedQuery = !_.isEmpty(args.include)
        const groupByQuery = !_.isEmpty(args.groupBy)

        if (nestedQuery && groupByQuery) {
          if (!args.groupBy.includes(this.uniqueColumn)) {
            throw new Error('Group by query with nested objects should contain the unique id column in group by field.')
          }
        }
      }
    }
